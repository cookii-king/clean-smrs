...
from ...models import PaymentLink, PaymentLinkLineItem, Account, Price

class PaymentLinkAdminForm(forms.ModelForm):
    """
    Custom form for managing PaymentLinks in the admin interface.
    """
    ...
    def save(self, commit=True):
        """
        Override save to link the customer to the payment_link.
        """
        instance = super().save(commit=False)
        print(f"override save items: ${instance.payment_link_line_items.all()}")
        if commit:
            instance.save()
        return instance

class PaymentLinkLineItemInline(admin.TabularInline):
    """
    Inline for managing PaymentLinkLineItem instances related to a PaymentLink.
    """
    model = PaymentLinkLineItem
    extra = 1
...
class PaymentLinkAdmin(admin.ModelAdmin):
    """
    Admin interface for managing PaymentLink.
    """
    ...
    inlines = [PaymentLinkLineItemInline]

    def save_model(self, request, obj, form, change):
        """
        Save the PaymentLink instance. Don't sync items here since they aren't saved yet.
        """
        super().save_model(request, obj, form, change)

    def save_related(self, request, form, formsets, change):
        """
        Save related PaymentLinkLineItems and synchronize with Stripe after all items are saved.
        """
        super().save_related(request, form, formsets, change)

        # Now that all related objects are saved, sync with Stripe
        payment_link = form.instance
        try:
            items = [
                {
            ...
            print(f"save_related payment_link.payment_link_line_items.all(): {payment_link.payment_link_line_items.all()}")

            if not items:
                self.message_user(request, "No items associated with this payment_link.", level='warning')
                return

            # Create or update Stripe payment_link
            stripe_payment_link = payment_link.create_or_update_stripe_payment_link()
            self.message_user(request, f"Stripe payment_link synced: {stripe_payment_link['id']}")
        except Exception as e:
            self.message_user(request, f"Error syncing Stripe payment_link: {e}", level='error')
